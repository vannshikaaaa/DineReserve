...
const btn = { width:"100%", padding:"13px", background:"#b89a6a", color:"#0c0d0f", border:"none", borderRadius:"8px", fontFamily:"'Jost', sans-serif", fontSize:"14px", fontWeight:"600", letterSpacing:"0.06em", textTransform:"uppercase", cursor:"pointer", marginTop:"6px", transition:"all 0.2s" };
...
function FI({ ...props }) {
  ...
  return <input {...props} style={{ ...inp, borderColor: f ? "#b89a6a" : "rgba(255,255,255,0.09)", boxShadow: f ? "0 0 0 3px rgba(184,154,106,0.12)" : "none" }} onFocus={() => setF(true)} onBlur={() => setF(false)} />;
}

export default function AdminRegister() {
  const [form, setForm] = useState({ email:"", password:"", confirm:"", restaurant_name:"", restaurant_unique_password:"" });
  ...
  const submit = async (e) => {
    ...
    try {
      await API.post("/admin/register", { email: form.email, password: form.password, restaurant_name: form.restaurant_name, restaurant_unique_password: form.restaurant_unique_password });
      ...
      <div style={wrap}>
        ...
        <div style={card}>
          ...
          <form onSubmit={submit}>
            <div style={field}><label style={label}>Email address</label><FI type="email" placeholder="admin@restaurant.com" onChange={set("email")} /></div>
            <div style={field}><label style={label}>Password</label><FI type="password" placeholder="••••••••" onChange={set("password")} /></div>
            <div style={field}><label style={label}>Confirm password</label><FI type="password" placeholder="••••••••" onChange={set("confirm")} /></div>
            <div style={{ height:"1px", background:"rgba(255,255,255,0.07)", margin:"20px 0" }} />
            <div style={field}><label style={label}>Restaurant name</label><FI placeholder="Must match exactly as in database" onChange={set("restaurant_name")} /></div>
            <div style={field}>
              ...
              <p style={{ fontSize:"12px", color:"#4a4844", marginTop:"6px" }}>The unique key set when your restaurant was created</p>
            </div>
            <button type="submit" style={btn} onMouseEnter={e => { e.currentTarget.style.background="#cdb48a"; e.currentTarget.style.transform="translateY(-1px)"; }} onMouseLeave={e => { e.currentTarget.style.background="#b89a6a"; e.currentTarget.style.transform="none"; }}>Register</button>
          </form>
          <div style={foot}><Link to="/admin-login" style={{ color:"#b89a6a", fontWeight:"500" }}>Back to sign in</Link></div>
        </div>
      </div>
    </div>
  );
