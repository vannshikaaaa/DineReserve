...
export default function ManageTables() {
  ...
  const load = async () => {
    setLoading(true);
    try { const r = await API.get("/admin/tables"); setTables(r.data); }
  ...
  return (
    <div style={{ minHeight:"100vh", background:BG, fontFamily:"'Jost', sans-serif", paddingBottom:"60px" }}>
      <div style={{ maxWidth:"860px", margin:"0 auto", padding:"40px 24px" }}>
        ...
        {/* Header */}
        <h1 style={{ fontFamily:SERIF, fontSize:"30px", fontWeight:"300", color:T1, letterSpacing:"-0.01em", marginBottom:"6px" }}>
          Manage Tables
        </h1>
        <p style={{ fontSize:"13px", color:T3, marginBottom:"32px" }}>Configure and monitor your restaurant floor</p>

        {error && (
          <div style={{ background:"rgba(196,96,96,0.1)", border:"1px solid rgba(196,96,96,0.2)", borderRadius:"8px", padding:"10px 16px", color:"#c46060", fontSize:"13px", marginBottom:"20px" }}>
            {error}
          </div>
        )}

        {/* Add table panel */}
        <div style={{ background:BG1, border:`1px solid ${BORDER_MD}`, borderRadius:"16px", padding:"26px", marginBottom:"28px" }}>
          <p style={{ fontSize:"11px", fontWeight:"500", color:T3, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:"18px" }}>
            Add new table
          </p>
          <div style={{ display:"flex", gap:"12px", flexWrap:"wrap", alignItems:"center" }}>
            ...
            <button
              onClick={add}
              style={{ padding:"12px 24px", background:GOLD, color:"#0c0d0f", border:"none",
                borderRadius:"8px", fontFamily:"'Jost', sans-serif", fontSize:"13px",
        ...
        {/* Table list */}
        {loading ? (
          <p style={{ color:T3, fontSize:"14px", textAlign:"center", padding:"40px" }}>Loading tables...</p>
        ) : tables.length === 0 ? (
            ...
            {tables.map(t => (
              ...
              >
                <div style={{ display:"flex", alignItems:"center", gap:"16px" }}>
                  {/* Table icon */}
                  <div style={{ width:"36px", height:"36px", background:BG3, border:`1px solid ${BORDER_MD}`,
                    borderRadius:"8px", display:"flex", alignItems:"center", justifyContent:"center",
                    fontSize:"13px", fontWeight:"600", color:GOLD, flexShrink:0 }}>
                    {t.name?.charAt(0)}
                  </div>
                  <div>
                    <p style={{ fontSize:"15px", fontWeight:"500", color:T1, marginBottom:"2px" }}>{t.name}</p>
                    <p style={{ fontSize:"12px", color:T3 }}>{t.seats} seat{t.seats !== 1 ? "s" : ""}</p>
                  </div>
                </div>
                <span style={{
