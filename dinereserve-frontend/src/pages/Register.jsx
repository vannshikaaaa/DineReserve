...
const cardTitle = { fontFamily:"'Cormorant Garamond', Georgia, serif", fontSize:"24px", fontWeight:"300", color:"#e8e4de", marginBottom:"28px" };
...
function Register() {
  ...
  const submit = async (e) => {
    ...
    try {
      await API.post("/customer/register", { name: form.name, email: form.email, password: form.password });
      navigate("/login");
      ...
      <div style={wrap}>
        ...
        <div style={card}>
          ...
          <form onSubmit={submit}>
            <div style={field}><label style={label}>Full name</label><FI placeholder="Your name" onChange={set("name")} /></div>
            <div style={field}><label style={label}>Email address</label><FI type="email" placeholder="you@example.com" onChange={set("email")} /></div>
          ...
          <div style={foot}>Already registered? <Link to="/login" style={{ color:"#b89a6a", fontWeight:"500" }}>Sign in</Link></div>
        </div>
      </div>
    </div>
  );
