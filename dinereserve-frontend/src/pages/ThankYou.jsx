...
function ThankYou() {
  ...
  useEffect(() => {
    ...
    setLoadingRecs(true);
    API.post("/ai/recommend-dish", { restaurant_id: booking.restaurant_id, food_preference: booking.food_preference || null, top_n: 5 })
    ...
    <div style={{ minHeight: "100vh", background: BG, fontFamily: SANS, paddingBottom: "60px" }}>
      <div style={{ maxWidth: "680px", margin: "0 auto", padding: "48px 24px" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: "36px" }}>
          ...
          <h1 style={{ fontFamily: SERIF, fontSize: "34px", fontWeight: "400", color: T1, marginBottom: "6px", letterSpacing: "-0.01em" }}>
            Booking confirmed
          </h1>
        ...
        <div style={{ background: S1, border: `1px solid ${BRD}`, borderRadius: "16px", padding: "24px", marginBottom: "28px" }}>
          {[
            ...
            <div key={label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderBottom: i < arr.length - 1 ? `1px solid ${BRD}` : "none" }}>
              <span style={{ fontSize: "11px", fontWeight: "500", letterSpacing: "0.07em", textTransform: "uppercase", color: T3 }}>{label}</span>
              <span style={{ fontSize: "14px", color: T2 }}>{val}</span>
            </div>
          ))}
        </div>
          ...
          {loadingRecs ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: "12px" }}>
              {recommendations.map((item, i) => (
                <div key={i}
                  style={{ background: S1, border: `1px solid ${BRD}`, borderRadius: "12px", padding: "16px", transition: "border-color 0.2s" }}
        ...
        <div style={{ display: "flex", gap: "12px" }}>
          <button onClick={() => navigate("/home")}
            style={{ padding: "12px 28px", background: GOLD, color: "#0e0f0d", border: "none", borderRadius: "10px", fontFamily: SANS, fontSize: "15px", fontWeight: "600", cursor: "pointer", transition: "all 0.2s" }}
          ...
          <button onClick={() => navigate("/bookings")}
            style={{ padding: "12px 24px", background: "transparent", color: T2, border: `1px solid ${BRD2}`, borderRadius: "10px", fontFamily: SANS, fontSize: "14px", cursor: "pointer", transition: "all 0.2s" }}
